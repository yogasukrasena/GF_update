...
//Include Common Files @1-A8C5AAF3
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "EditEngobe.php");
...
//Class_Initialize Event @2-1EF1E54D
    function clsRecordtblengobe($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->EngobeDate = new clsControl(ccsTextBox, "EngobeDate", "Engobe Date", ccsDate, $DefaultDateFormat, CCGetRequestParam("EngobeDate", $Method, NULL), $this);
            $this->EngobeDate->Required = true;
            $this->DatePicker_EngobeDate = new clsDatePicker("DatePicker_EngobeDate", "tblengobe", "EngobeDate", $this);
            $this->EngobeNotes = new clsControl(ccsTextArea, "EngobeNotes", "Engobe Notes", ccsMemo, "", CCGetRequestParam("EngobeNotes", $Method, NULL), $this);
...
//Validate Method @2-5B13300D
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        $Where = "";
        $Validation = ($this->EngobeDate->Validate() && $Validation);
        $Validation = ($this->EngobeNotes->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->EngobeDate->Errors->Count() == 0);
        $Validation =  $Validation && ($this->EngobeNotes->Errors->Count() == 0);
...
//CheckErrors Method @2-FDE8DECA
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->EngobeCode->Errors->Count());
        $errors = ($errors || $this->EngobeDescription->Errors->Count());
        $errors = ($errors || $this->EngobeDate->Errors->Count());
        $errors = ($errors || $this->DatePicker_EngobeDate->Errors->Count());
...
//Operation Method @2-32AE786A
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = "Engobe.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "ID", "EngobeCode"));
...
//UpdateRow Method @2-1A2C0522
    function UpdateRow()
    {
        ...
        $this->DataSource->EngobeDate->SetValue($this->EngobeDate->GetValue(true));
        $this->DataSource->EngobeNotes->SetValue($this->EngobeNotes->GetValue(true));
    ...
    function Show()
    {
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    $this->EngobeDate->SetValue($this->DataSource->EngobeDate->GetValue());
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->EngobeDate->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DatePicker_EngobeDate->Errors->ToString());
        ...
        $this->EngobeDate->Show();
        $this->DatePicker_EngobeDate->Show();
...
//DataSource Variables @2-2D2A93D6
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-B3F05990
    function clstblengobeDataSource(& $Parent)
    {
        ...
        $this->EngobeDate = new clsField("EngobeDate", ccsDate, $this->DateFormat);
        ...
        $this->UpdateFields["EngobeDate"] = array("Name" => "EngobeDate", "Value" => "", "DataType" => ccsDate, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobeNotes"] = array("Name" => "EngobeNotes", "Value" => "", "DataType" => ccsMemo, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobeTechDraw"] = array("Name" => "EngobeTechDraw", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobePhoto1"] = array("Name" => "EngobePhoto1", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobePhoto2"] = array("Name" => "EngobePhoto2", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobePhoto3"] = array("Name" => "EngobePhoto3", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["EngobePhoto4"] = array("Name" => "EngobePhoto4", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
    }
...
//SetValues Method @2-D5B71127
    function SetValues()
    {
        $this->EngobeCode->SetDBValue($this->f("EngobeCode"));
        $this->EngobeDescription->SetDBValue($this->f("EngobeDescription"));
        $this->EngobeDate->SetDBValue(trim($this->f("EngobeDate")));
        $this->EngobeNotes->SetDBValue($this->f("EngobeNotes"));
        $this->FileUpload1->SetDBValue($this->f("EngobeTechDraw"));
        $this->FileUpload2->SetDBValue($this->f("EngobePhoto1"));
        $this->FileUpload3->SetDBValue($this->f("EngobePhoto2"));
        $this->FileUpload4->SetDBValue($this->f("EngobePhoto3"));
        $this->FileUpload5->SetDBValue($this->f("EngobePhoto4"));
    }
//End SetValues Method

//Update Method @2-2E0E7412
    function Update()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->UpdateFields["EngobeDate"]["Value"] = $this->EngobeDate->GetDBValue(true);
        $this->UpdateFields["EngobeNotes"]["Value"] = $this->EngobeNotes->GetDBValue(true);
        $this->UpdateFields["EngobeTechDraw"]["Value"] = $this->FileUpload1->GetDBValue(true);
        $this->UpdateFields["EngobePhoto1"]["Value"] = $this->FileUpload2->GetDBValue(true);
        $this->UpdateFields["EngobePhoto2"]["Value"] = $this->FileUpload3->GetDBValue(true);
        $this->UpdateFields["EngobePhoto3"]["Value"] = $this->FileUpload4->GetDBValue(true);
        $this->UpdateFields["EngobePhoto4"]["Value"] = $this->FileUpload5->GetDBValue(true);
...
//Delete Method @2-8B78B80F
    function Delete()
    {
        ...
        $this->SQL = "DELETE FROM tblengobe";
        $this->SQL = CCBuildSQL($this->SQL, $this->Where, "");
        if (!strlen($this->Where) && $this->Errors->Count() == 0) 
            $this->Errors->addError($CCSLocales->GetText("CCS_CustomOperationError_MissingParameters"));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "EditEngobe.html";
$BlockToParse = "main";
