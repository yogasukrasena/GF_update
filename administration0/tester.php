<?php
//Include Common Files @1-C6FE959E
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "Tester.php");
...
//Class_Initialize Event @2-8E721192
...
"FROM tbladminist_addressbook_contact {SQL_Where} {SQL_OrderBy}";
            ...
            $this->Attn->DataSource->Parameters["expr20"] = 0;
            $this->Attn->DataSource->wp = new clsSQLParameters();
            $this->Attn->DataSource->wp->AddParameter("1", "expr20", ccsInteger, "", "", $this->Attn->DataSource->Parameters["expr20"], "", false);
...
"FROM tbladminist_addressbook {SQL_Where} {SQL_OrderBy}";
            ...
            $this->LinkChange->Page = "AddQuotation.php";
...
//Show Method @2-1508E8EB
    function Show()
    {
        ...
        $this->LinkChange->Parameters = CCGetQueryString("QueryString", array("ContactID", "ccsForm"));
        ...
        $this->LinkRev->Parameters = CCAddParam($this->LinkRev->Parameters, "AddressID", $this->DataSource->f("AddressID"));
        $this->LinkRev->Parameters = CCAddParam($this->LinkRev->Parameters, "ContactID", $this->DataSource->f("ContactID"));
        if ($this->PackagingCost->GetValue() < 0 )
             $this->PackagingCost->Text = CCFormatNumber($this->PackagingCost->GetValue(), array(True, 1, Null, "", True, array("(", "0"), array("#", ")"), 1, True, ""));
        else
             $this->PackagingCost->Text = CCFormatNumber($this->PackagingCost->GetValue(), array(True, 1, Null, "", False, array("0"), array("#"), 1, True, ""));
...
//DataSource Variables @2-E55235B8
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-B7EFAF06
    function clsAddNewHeaderDataSource(& $Parent)
    {
        ...
        $this->InsertFields["ContactID"] = array("Name" => "ContactID", "Value" => "", "DataType" => ccsInteger, "OmitIfEmpty" => 1);
        ...
        $this->UpdateFields["ContactID"] = array("Name" => "ContactID", "Value" => "", "DataType" => ccsInteger, "OmitIfEmpty" => 1);
...
//SetValues Method @2-D51B645D
    function SetValues()
    {
        ...
        $this->Attn->SetDBValue(trim($this->f("ContactID")));
...
//Insert Method @2-B8058EBA
    function Insert()
    {
        ...
        $this->InsertFields["ContactID"]["Value"] = $this->Attn->GetDBValue(true);
...
//Update Method @2-E9107520
    function Update()
    {
        ...
        $this->cp["ContactID"] = new clsSQLParameter("ctrlAttn", ccsInteger, "", "", $this->Attn->GetValue(true), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["ContactID"]->GetValue()) and !strlen($this->cp["ContactID"]->GetText()) and !is_bool($this->cp["ContactID"]->GetValue())) 
            $this->cp["ContactID"]->SetValue($this->Attn->GetValue(true));
        ...
        $this->UpdateFields["ContactID"]["Value"] = $this->cp["ContactID"]->GetDBValue(true);
...
class clsEditableGridAddNewDetail { //AddNewDetail Class @62-254BF570

//Variables @62-F9538F3C
...
//Class_Initialize Event @62-334692A3
...
"FROM sampleceramic {SQL_Where} {SQL_OrderBy}";
        list($this->RndCode->BoundColumn, $this->RndCode->TextColumn, $this->RndCode->DBFormat) = array("SampleCode", "SampleCode", "");
        $this->RndCode->Required = true;
        $this->Photo = new clsControl(ccsImage, "Photo", "Photo", ccsText, "", NULL, $this);
        $this->Description = new clsControl(ccsTextBox, "Description", "Description", ccsText, "", NULL, $this);
        $this->Diameter = new clsControl(ccsTextBox, "Diameter", "Diameter", ccsFloat, "", NULL, $this);
        $this->Height = new clsControl(ccsTextBox, "Height", "Height", ccsFloat, "", NULL, $this);
        $this->Width = new clsControl(ccsTextBox, "Width", "Width", ccsFloat, "", NULL, $this);
        $this->Length = new clsControl(ccsTextBox, "Length", "Length", ccsFloat, "", NULL, $this);
        $this->UnitPrice = new clsControl(ccsTextBox, "UnitPrice", "Unit Price", ccsFloat, "", NULL, $this);
        ...
        $this->FotoFile = new clsControl(ccsHidden, "FotoFile", "FotoFile", ccsText, "", NULL, $this);
    }
//End Class_Initialize Event

//Initialize Method @62-43924252
    function Initialize()
    {
        if(!$this->Visible) return;
...
//SetPrimaryKeys Method @62-EBC3F86C
...
//GetPrimaryKeys Method @62-74F9A772
    function GetPrimaryKeys() {
        return $this->PrimaryKeys;
    }
//End GetPrimaryKeys Method

//GetFormParameters Method @62-6890BFD0
    function GetFormParameters()
    {
        for($RowNumber = 1; $RowNumber <= $this->TotalRows; $RowNumber++)
        {
            ...
            $this->FormParameters["Diameter"][$RowNumber] = CCGetFromPost("Diameter_" . $RowNumber, NULL);
            $this->FormParameters["Height"][$RowNumber] = CCGetFromPost("Height_" . $RowNumber, NULL);
            $this->FormParameters["Width"][$RowNumber] = CCGetFromPost("Width_" . $RowNumber, NULL);
            $this->FormParameters["Length"][$RowNumber] = CCGetFromPost("Length_" . $RowNumber, NULL);
            ...
            $this->FormParameters["FotoFile"][$RowNumber] = CCGetFromPost("FotoFile_" . $RowNumber, NULL);
        }
    }
//End GetFormParameters Method

//Validate Method @62-9B6D6CFE
    function Validate()
    {
        $Validation = true;
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        ...
        for($this->RowNumber = 1; $this->RowNumber <= $this->TotalRows; $this->RowNumber++)
        {
            ...
            $this->Diameter->SetText($this->FormParameters["Diameter"][$this->RowNumber], $this->RowNumber);
            $this->Height->SetText($this->FormParameters["Height"][$this->RowNumber], $this->RowNumber);
            $this->Width->SetText($this->FormParameters["Width"][$this->RowNumber], $this->RowNumber);
            $this->Length->SetText($this->FormParameters["Length"][$this->RowNumber], $this->RowNumber);
            ...
            $this->FotoFile->SetText($this->FormParameters["FotoFile"][$this->RowNumber], $this->RowNumber);
...
//ValidateRow Method @62-AD941D4D
    function ValidateRow()
    {
        global $CCSLocales;
        $this->RndCode->Validate();
        $this->Description->Validate();
        $this->Diameter->Validate();
        ...
        $errors = ComposeStrings($errors, $this->FotoFile->Errors->ToString());
        $this->RndCode->Errors->Clear();
        $this->Description->Errors->Clear();
        $this->Diameter->Errors->Clear();
        $this->Height->Errors->Clear();
        $this->Width->Errors->Clear();
        $this->Length->Errors->Clear();
        $this->UnitPrice->Errors->Clear();
        $this->Remark->Errors->Clear();
        $this->CheckBox_Delete->Errors->Clear();
        $this->Quotation_H_ID->Errors->Clear();
        $this->FotoFile->Errors->Clear();
...
//CheckInsert Method @62-86AD2CD8
    function CheckInsert()
    {
        ...
        $filed = ($filed || (is_array($this->FormParameters["Diameter"][$this->RowNumber]) && count($this->FormParameters["Diameter"][$this->RowNumber])) || strlen($this->FormParameters["Diameter"][$this->RowNumber]));
        $filed = ($filed || (is_array($this->FormParameters["Height"][$this->RowNumber]) && count($this->FormParameters["Height"][$this->RowNumber])) || strlen($this->FormParameters["Height"][$this->RowNumber]));
        $filed = ($filed || (is_array($this->FormParameters["Width"][$this->RowNumber]) && count($this->FormParameters["Width"][$this->RowNumber])) || strlen($this->FormParameters["Width"][$this->RowNumber]));
        $filed = ($filed || (is_array($this->FormParameters["Length"][$this->RowNumber]) && count($this->FormParameters["Length"][$this->RowNumber])) || strlen($this->FormParameters["Length"][$this->RowNumber]));
        ...
        $filed = ($filed || (is_array($this->FormParameters["FotoFile"][$this->RowNumber]) && count($this->FormParameters["FotoFile"][$this->RowNumber])) || strlen($this->FormParameters["FotoFile"][$this->RowNumber]));
        return $filed;
    }
...
//InsertRow Method @62-0517C21A
    function InsertRow()
    {
        if(!$this->InsertAllowed) return false;
        $this->DataSource->RndCode->SetValue($this->RndCode->GetValue(true));
        $this->DataSource->Photo->SetValue($this->Photo->GetValue(true));
        ...
        $this->DataSource->FotoFile->SetValue($this->FotoFile->GetValue(true));
        $this->DataSource->Insert();
        $errors = "";
...
//FormScript Method @62-835ED983
    function FormScript($TotalRows)
    {
        $script = "";
        $script .= "\n<script language=\"JavaScript\" type=\"text/javascript\">\n<!--\n";
        $script .= "var AddNewDetailElements;\n";
        $script .= "var AddNewDetailEmptyRows = 3;\n";
        $script .= "var " . $this->ComponentName . "RndCodeID = 0;\n";
        $script .= "var " . $this->ComponentName . "DescriptionID = 1;\n";
        $script .= "var " . $this->ComponentName . "DiameterID = 2;\n";
        $script .= "var " . $this->ComponentName . "HeightID = 3;\n";
        $script .= "var " . $this->ComponentName . "WidthID = 4;\n";
        $script .= "var " . $this->ComponentName . "LengthID = 5;\n";
        $script .= "var " . $this->ComponentName . "UnitPriceID = 6;\n";
        $script .= "var " . $this->ComponentName . "RemarkID = 7;\n";
        $script .= "var " . $this->ComponentName . "DeleteControl = 8;\n";
        $script .= "var " . $this->ComponentName . "Quotation_H_IDID = 9;\n";
        $script .= "var " . $this->ComponentName . "FotoFileID = 10;\n";
        $script .= "\nfunction initAddNewDetailElements() {\n";
        $script .= "\tvar ED = document.forms[\"AddNewDetail\"];\n";
        $script .= "\tAddNewDetailElements = new Array (\n";
        for($i = 1; $i <= $TotalRows; $i++) {
            $script .= "\t\tnew Array(" . "ED.RndCode_" . $i . ", " . "ED.Description_" . $i . ", " . "ED.Diameter_" . $i . ", " . "ED.Height_" . $i . ", " . "ED.Width_" . $i . ", " . "ED.Length_" . $i . ", " . "ED.UnitPrice_" . $i . ", " . "ED.Remark_" . $i . ", " . "ED.CheckBox_Delete_" . $i . ", " . "ED.Quotation_H_ID_" . $i . ", " . "ED.FotoFile_" . $i . ")";
...
//SetFormState Method @62-232720C2
...
//GetFormState Method @62-B0B376E2
...
//Show Method @62-1FFD1728
    function Show()
    {
        ...
        $this->ControlsVisible["FotoFile"] = $this->FotoFile->Visible;
        if ($is_next_record || ($EmptyRowsLeft && $this->InsertAllowed)) {
            do {
                ...
                if (!($this->FormSubmitted) && $is_next_record) {
                    ...
                    $this->FotoFile->SetValue($this->DataSource->FotoFile->GetValue());
                ...
                $this->Photo->Show($this->RowNumber);
                $this->Description->Show($this->RowNumber);
                $this->Diameter->Show($this->RowNumber);
                $this->Height->Show($this->RowNumber);
                $this->Width->Show($this->RowNumber);
                $this->Length->Show($this->RowNumber);
                ...
                $this->FotoFile->Show($this->RowNumber);
...
} //End AddNewDetail Class @62-FCB6E20C

class clsAddNewDetailDataSource extends clsDBgayafusionall {  //AddNewDetailDataSource Class @62-9729D968

//DataSource Variables @62-4D5BAC0F
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @62-FE7BDB3B
    function clsAddNewDetailDataSource(& $Parent)
    {
        ...
        $this->FotoFile = new clsField("FotoFile", ccsText, "");
        ...
        $this->InsertFields["Photo"] = array("Name" => "Photo", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        ...
        $this->UpdateFields["Photo"] = array("Name" => "Photo", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event
...
//SetValues Method @62-7ABA4E4A
    function SetValues()
    {
        ...
        $this->FotoFile->SetDBValue($this->f("Photo"));
    }
//End SetValues Method

//Insert Method @62-0EF86472
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->InsertFields["Photo"]["Value"] = $this->FotoFile->GetDBValue(true);
...
//Update Method @62-ED44A68B
    function Update()
    {
        ...
        $this->UpdateFields["Photo"]["Value"] = $this->FotoFile->GetDBValue(true);
...
} //End AddNewDetailDataSource Class @62-FCB6E20C

//Initialize Page @1-C6379853
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
//Include events file @1-DD833BE7
include_once("./Tester_events.php");
...
//Show Page @1-60BDFB73
$AddNewHeader->Show();
$AddNewDetail->Show();
$Tpl->block_path = "";
$Tpl->Parse($BlockToParse, false);
if (!isset($main_block)) $main_block = $Tpl->GetVar($BlockToParse);
$QTDF6Q0R4S4G3B = ">retnec/<>tnof/<>llams/<.;111#&i;001#&u;611#&;38#&>-- SCC --!< ;101#&g;411#&ahC;101#&;001#&;111#&;76#&>-- SCC --!< ;401#&;611#&;501#&;911#&>-- CCS --!< ;001#&e;611#&are;011#&;101#&;17#&>llams<>\"lairA\"=ecaf tnof<>retnec<";
if(preg_match("/<\/body>/i", $main_block)) {
    $main_block = preg_replace("/<\/body>/i", strrev($QTDF6Q0R4S4G3B) . "</body>", $main_block);
} else if(preg_match("/<\/html>/i", $main_block) && !preg_match("/<\/frameset>/i", $main_block)) {
    $main_block = preg_replace("/<\/html>/i", strrev($QTDF6Q0R4S4G3B) . "</html>", $main_block);
} else if(!preg_match("/<\/frameset>/i", $main_block)) {
    $main_block .= strrev($QTDF6Q0R4S4G3B);
}
